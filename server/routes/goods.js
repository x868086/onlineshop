...
mongoose.connect(encodeURI('mongodb://shopadmin:123456@127.0.0.1:27017/unishop'), { useNewUrlParser: true });
...
router.get("/", (req, res, next) => {
  //1.获取前端发起的查询参数
  const sortFlag = req.param('sortFlag');
  const pageNum = Number(req.param('pageNum'));
  const pageSize = Number(req.param('pageSize'));
  //1.1根据页数和每页的数量计算要跳过的条数
  const skips = (pageNum - 1) * pageSize
  const conditions = req.param('condition')
  const minvalue = req.param("minvalue")
  const maxvalue = req.param("maxvalue")
  const obj = {
  ...
  const modelQuery = Goods.find(queryParam).skip(skips).limit(pageSize);
    ...
    if(err) {
      ...
      Goods.findOne({ productId: productId }).exec((err1, doc) => {
        if(err1) {
          ...
          const inItem = data.cartList.find((e, i, a) => {
